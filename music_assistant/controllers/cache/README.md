...
- The `@use_cache` decorator accepts `allow_expired_cache=True` to enable stale-while-revalidate: an expired entry is returned immediately and a background refresh updates the cache for the next request. The `BYPASS_CACHE` context variable still forces a synchronous re-fetch. An expired *empty* collection is the exception: it is not useful fallback data (it would otherwise resolve, for example, a playlist to nothing and fail playback), so it is treated as a miss and fetched synchronously.
